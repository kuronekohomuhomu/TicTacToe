...
def masume():
    ...
    for y in range(3):
        for x in range(3):
            ...
            Y = y * 200
            if masu[y][x] == 1:
                cvs.create_oval(X+20, Y+20, X+180, Y+180, outline="skyblue", width=30)
            if masu[y][x] == 2:
                cvs.create_line(X+20, Y+20, X+180, Y+180, fill="pink", width=24)
                cvs.create_line(X+180, Y+20, X+20, Y+180, fill="pink", width=24)
    if shirushi == 0:
